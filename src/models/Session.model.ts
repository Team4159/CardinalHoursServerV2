...
async function getSessionsByPassword(password: number): Promise<Session[]> {
 const sql = "SELECT * FROM `sessions` WHERE user_id = (SELECT user_id from users WHERE password = ? )"
...
async function createSession(
 ...
 if (amended) {
   sql = "INSERT INTO `sessions` (user_id, start_time, end_time, amended) SELECT user_id, ?, ?, ? FROM users WHERE password = ?)";
...
async function updateSession(password: number, end_time: bigint): Promise<boolean> {
 const sql = "UPDATE `sessions` SET end_time = ? WHERE user_id = (SELECT user_id from 'users' WHERE password = ?)";
 const [ resHeader ] =  await database.query<ResultSetHeader>(sql, [ end_time, password ]);
